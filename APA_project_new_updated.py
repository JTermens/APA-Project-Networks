...
class Tree(object):
    def __init__(self,node,left=None,right=None, axis_key= None):
        ...
        self.right = right
        self.axis = axis_key
        #print('this is self node',node)

    def group(self, x, d, k, *argv):

        #kd_tree = self
        axis_key = kd_tree.axis_key
        ...
        if len(axis_key) != len(argv):
            ...
            print('Fiding the %s nearest neighbours...' %(num_neighbours))
...
if __name__=='__main__':
    ...
    for f in range(0,len(final)):
    	print ("dist = {}; Community: dens = {}, size = {}, max btw = {}, ...)".format(final[f][0],final[f][1].dens,final[f][1].size,final[f][1].max_btw))
